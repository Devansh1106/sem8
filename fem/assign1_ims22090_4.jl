#=      -4*∇^2 y + ∇y + 3*y = f(x), x ∈ (0,1) with y(0) = 0 = y(1)
        Ans(a): Exact sol: sin(π*x); f(x) = (π^2 + 4)* sin(π*x)
=#

using LinearAlgebra, Plots
function f(x)
    return 4 * π^2 * sin(π*x) + π * cos(π * x) + 3 * sin(π * x)
end
...
function fem1d(N)
    ...
    for i in 1:N+1
        A[Elem[i,:], Elem[i,:]] += [4/h -4/h; -4/h 4/h] + [h h/2; h/2 h] + [-1/2 1/2; -1/2 1/2]
        midpt = (x[i] + x[i+1])/2
